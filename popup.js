...
async function loadInitialData() {
    setLoadingState(SELECTORS.reloadBtn, true);
    ...
    try {
        ...
        setLoadingState(SELECTORS.reloadBtn, false);
    }
}
...
function downloadPDF() {
    ...
    setLoadingState(SELECTORS.downloadBtn, true);
    
    // Use timeout to show loading state even if PDF generation is fast
    setTimeout(() => {
        try {
            if (!window.jspdf) {
            ...
            setLoadingState(SELECTORS.downloadBtn, false);
        }
    }, 500);
}
...
async function copySummary() {
    ...
    setLoadingState(SELECTORS.copyBtn, true);
    ...
    try {
        ...
        setLoadingState(SELECTORS.copyBtn, false);
    }
}
...
function calculateProjection() {
    ...
    setLoadingState('#calculateProjection', true);
    ...
    setTimeout(() => {
        try {
            ...
            showMessage('Projection calculated successfully!', 'success');
        } catch (error) {
            showMessage('Error calculating projection.', 'error');
            console.error('Projection error:', error);
        } finally {
            setLoadingState('#calculateProjection', false);
        }
    }, 800);
}
